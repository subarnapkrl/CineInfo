...
export function MovieDetail({title}) {
  ...
  useEffect(()=>{
    async function fetchMovie(){
      const response=await fetch(`https://api.themoviedb.org/3/movie/${params.id}?api_key=425c2b1ee5d1bc874cf73844d4991621`);
  ...
  useEffect(()=>{
    document.title=`${data.title} / CineInfo`
  })
  return (
    <main>
      <section className='flex justify-around flex-wrap py-5'>
        <div className='max-w-sm'>
            <img className='rounded' src={image} alt={MovieDetail.title} />
        </div>
